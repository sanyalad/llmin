...
def make_episode(*, minimum_retain_until: datetime) -> Episode:
    return Episode(
        ...
        attempt_id=uuid4(),
        summary=_SUMMARY,
        content_hash=episode_content_hash(_SUMMARY),
...
def persist_episode_provenance(store: SQLiteMemoryStore, episode: Episode) -> None:
    event_id = next(iter(episode.source_event_ids))
    evidence_id = next(iter(episode.evidence_ids))
    store.append_trace(
        TraceEvent(
            event_id=event_id,
            trace_id=uuid4(),
...
def test_episode_lifecycle_preserves_auditable_tombstone(tmp_path: Path) -> None:
    ...
    assert stored.source_event_ids == episode.source_event_ids
